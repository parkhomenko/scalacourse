...
class Html extends JavaTokenParsers {
  ...
  def doubleTag: Parser[(String, Any)] = "<"~tagName~">"~content~"</"~tagName~">" ^^
    { case "<"~openTagName~">"~content~"</"~closeTagName~">" => (openTagName, content)}
  def content: Parser[Any] = rep(doubleTag) | textContent
  def document: Parser[Any] = doctype~htmlTagOpen~bodyTagOpen~content~bodyTagClose~htmlTagClose ^^
    { case doctype~htmlTagOpen~bodyTagOpen~content~bodyTagClose~htmlTagClose => content}
}
